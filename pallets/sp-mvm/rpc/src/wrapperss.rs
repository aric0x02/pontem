...
pub struct IdentifierWrapper(pub String);

impl VerifyInput for IdentifierWrapper {
    fn verify(&self) -> anyhow::Result<()> {
        if Identifier::is_valid(self.0.as_str()) {
            Ok(())
        } else {
            bail!("String is invalid {}", self)
        }
    }
}
...
impl FromStr for IdentifierWrapper {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        Ok(IdentifierWrapper(String::from_str(s)?))
    }
}
...
//         Self(ident_str.as_str().to_string())
//     }
// }

// impl AsRef<IdentStr> for IdentifierWrapper {
//     fn as_ref(&self) -> &IdentStr {
//         Identifier::new(self.0.as_ref()).unwrap().as_ref()
//     }
// }
...
//         Identifier::new(self.0.as_ref()).unwrap().deref()
//     }
// }
